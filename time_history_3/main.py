...
Res = 2.428
Ind = 3.6829e-4
Voltage = SensorFilter(2*pi*Res/Ind, n_act, idt, 0)
...
for step in range(20):
  ...
  while itime <= T: 
    ...
    #Voltage.Integrate(-2.026*mnt.ud.sub(0).compute_vertex_values()[act.vertex_id]+curr/Ind)
    #f_filt = act.current2force_all(Voltage.x, mnt.u, offx , offy)
	...
	#ff.uold[:] = ff.u[:]
      ...
      #####filtro la posizione e faccio le differenze finite --qui devo lavorare
...
np.savetxt('u.dat', np.append(tv*1000,trace[0]*1e9).reshape(2,np.size(tv)).transpose())
np.savetxt('utrue.dat', np.append(tv*1000,tracetrue[0]*1e9).reshape(2,np.size(tv)).transpose())
np.savetxt('ud.dat', np.append(tv*1000,tracevel[0]*1e3).reshape(2,np.size(tv)).transpose())
np.savetxt('udtrue.dat', np.append(tv*1000,traceveltrue[0,:]*1e3).reshape(2,np.size(tv)).transpose())
