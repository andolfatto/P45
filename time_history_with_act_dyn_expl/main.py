...
ones = np.ones(pippo.vector().size())
mdiag.vector()[:] = np.divide(ones,pippo.vector())
M.zero()
...
for step in range(2):
  ...
  while itime <= T: 
    ...
    if itime >= time-itime/10:  
      ...
      trace = np.transpose(np.append(np.transpose(trace), np.transpose(act.disp2disp_all(ab2.u))).\
	      reshape(np.shape(trace)[1]+1,np.shape(trace)[0]))
      tracetrue = np.transpose(np.append(np.transpose(tracetrue), np.transpose(ab2.u.sub(0).compute_vertex_values()[act.vertex_id])).\
      ...
      traceveltrue = np.transpose(np.append(np.transpose(traceveltrue), np.transpose(ab2.ud.sub(0).compute_vertex_values()[act.vertex_id])).\
...
#embed()
np.savetxt("results/ref2.dat",ref[0,:])
np.savetxt("results/refd2.dat",refvel[0,:])
np.savetxt("results/u2.dat",trace[0,:])
np.savetxt("results/utrue2.dat",tracetrue[0,:])
np.savetxt("results/ud2.dat",tracevel[0,:])
np.savetxt("results/udtrue2.dat",traceveltrue[0,:])
np.savetxt("results/time2.dat",tv)
np.savetxt("results/orizzontal2.dat",np.transpose(np.append(offx,offy).reshape(2,n_act)))
fig = plt.figure()
