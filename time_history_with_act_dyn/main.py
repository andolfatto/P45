...
#fcost = -np.sum(assemble(forcew(Constant(9.81*rho*t),test,dx)).array())/n_act+0.2507-9.81*4.3201e-3
#############################################################################################
...
#for step in range(20):
  ...
  while itime <= T: 
	...
	#foxystoring = np.transpose(np.append(np.transpose(foxystoring), np.transpose(oforce)).\
	      #reshape(oxycount+1,n_act))
...
np.savetxt('7u.dat', np.append(tv*1000,trace[0]*1e9).reshape(2,np.size(tv)).transpose())
np.savetxt('7utrue.dat', np.append(tv*1000,tracetrue[0]*1e9).reshape(2,np.size(tv)).transpose())
np.savetxt('7ud.dat', np.append(tv*1000,tracevel[0]*1e9).reshape(2,np.size(tv)).transpose())
np.savetxt('7udtrue.dat', np.append(tv*1000,traceveltrue[0]*1e9).reshape(2,np.size(tv)).transpose())

#np.savetxt("results/ref5.dat",ref[0,:])
#np.savetxt("results/refd5.dat",refvel[0,:])
#np.savetxt("results/u5.dat",trace[0,:])
#np.savetxt("results/utrue5.dat",tracetrue[0,:])
#np.savetxt("results/ud5.dat",tracevel[0,:])
#np.savetxt("results/udtrue5.dat",traceveltrue[0,:])
#np.savetxt("results/time5.dat",tv)
#np.savetxt("results/orizzontal5.dat",np.transpose(np.append(offx,offy).reshape(2,n_act)))
fig = plt.figure()
