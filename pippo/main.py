...
#for step in range(30):
  ...
  while itime <= T:
    ...
    if itime >= time-itime/10:
        ...
        #oxystoring = np.transpose(np.append(np.transpose(oxystoring),\
	#np.transpose(utemp1.sub(0).compute_vertex_values()[act.vertex_id])).\
...
np.savetxt('2u.dat', np.append(tv*1000,trace[0]*1e9).reshape(2,np.size(tv)).transpose())
np.savetxt('2utrue.dat', np.append(tv*1000,tracetrue[0]*1e9).reshape(2,np.size(tv)).transpose())
