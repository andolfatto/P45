...
#Lumped_K_tensor = as_tensor([    [ -87.4 , 0.          , 0.           ],\
                                 #[ 0.           , 7e-5 , 0.           ],\
                                 #[ 0.           , 0.         ,  7e-5 ]])
Lumped_K_tensor = as_tensor([    [ 0.             , 0.        , 0.    ],\
                                 [ 0.           , 7e-5 , 0.           ],\
                                 [ 0.           , 0.          ,  7e-5 ]])
...
sens = SensorFilter(1000, n_act, idt, 3.188993986668676e-11)
filt = SensorFilter(800, n_act, idt, 0)
tv = np.array([0])
my_filter_for_vel = SensorFilter(800, n_act, idt, 0)
...
for step in range(3):
  ...
  while itime <= T: 
    ...
    u = mnt.u.sub(0).compute_vertex_values()[act.vertex_id]
...
np.savetxt('ud.dat', np.append(tv*1000,tracevel[0]).reshape(2,np.size(tv)).transpose())
np.savetxt('udtrue.dat', np.append(tv*1000,traceveltrue[0]).reshape(2,np.size(tv)).transpose())
