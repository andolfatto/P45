...
#t0 = 0
#time = t0
#fexpr = Expression ("5",t = t0)#("50*sin(1e+3*t)",t=t0)
#u0 = Function(L)
#u0.vector()[:] = 0
#udot0 = u0
## time step
...
#u1 = Function ( L )
#T = 100*dt
#p = plot(u0.sub(0),title="w", range_max = 0.001 , range_min = -0.001,\
	 #window_width = 1920 , window_height = 1280)
##cycle
#while time <= T:
  #fexpr . t = time
  #b = assemble ( rhs )
  #bc . apply ( b )
  #B = b+M/(dt*dt)*u0.vector()+M/dt*udot0.vector()
  #solve(M/(dt*dt)+K, u1.vector(), B)
  #time += dt
  #print time
...
print str(time) + 'eulero'
fexpr . t = time
F = assemble ( rhs )
bc . apply ( F )
right = F+M/(dt*dt)*u0.vector()+M/dt*v0.vector()
solve(M/(dt*dt)+K, u1.vector(), right)
v1.assign ( (u1-u0)/dt )
a1.assign ( (v1-v0)/dt )
#p.plot(u1.sub(0))
time += dt
...
#PETSc.Viewer().view(as_backend_type(left).mat())
#asd
#CS = CholmodCholeskySolver(left)
#CS = UmfpackLUSolver(left)
LU = LUSolver(left)
...
while time <= T:
  ...
  #solve(left, a2.vector(), right, 'umfpack')
